...
Recall from [chapter 1](01_inference.md) that we defined the language model as taking state, along with a list of integer token indices and returning an updated state and real-valued logit vector of shape $\mathtt{[V]}$---one entry for each token in the vocabulary. This gave us the type annotation:
...
```catform
main(weights: *, state: *, tokens: i32[N]) -> (state: *, logits: bf16[param.vocab])
```
...
\node at (0,1.5) {\large $\mathtt{forward}$};
\node at (0,-0.75) {\small $\mathtt{transformer}$};
...
\draw[thick, rounded corners] (2.4,-0.5) rectangle (3.6,0.5);
\draw[thick, rounded corners] (0.6,-0.5) rectangle (1.8,0.5);
\draw[thick, rounded corners] (-0.6,-0.5) rectangle (-1.8,0.5);
\draw[thick, rounded corners] (-2.4,-0.5) rectangle (-3.6,0.5);
...
\node at (0,1.5) {\large $\mathtt{layer}$};
\draw[thick, rounded corners] (-6.0,-2.0) rectangle (6.0,2.0);

\draw[thick] (-7,0.6) -- (7,0.6) ;

\draw[thick, rounded corners] (-5.1,-1.1) rectangle (-3.4,-0.1);
\draw[thick, rounded corners] (-2.7,-1.1) rectangle (-0.8,-0.1);
\node at (-4.25,-0.6) {$\mathtt{rmsnorm}$};
\node at (-1.75,-0.6) {$\mathtt{attention}$};

\draw[thick, rounded corners] (-5.6,0.6) -- (-5.6,-0.6) -- (-5.1,-0.6) ;
\draw[thick] (-3.4,-0.6) -- (-2.7,-0.6) ;
\draw[thick, rounded corners] (-0.8,-0.6) -- (-0.2,-0.6) -- (-0.2,0.6) ;

\draw[thick, rounded corners] ( 0.8,-1.1) rectangle ( 2.5,-0.1);
\draw[thick, rounded corners] ( 3.2,-1.1) rectangle ( 5.1,-0.1);
\node at ( 1.65,-0.6) {$\mathtt{rmsnorm}$};
\node at ( 4.15,-0.6) {$\mathtt{ffn}$};

\draw[thick, rounded corners] ( 0.2,0.6) -- ( 0.2,-0.6) -- ( 0.8,-0.6) ;
\draw[thick] ( 2.5,-0.6) -- ( 3.2,-0.6) ;
\draw[thick, rounded corners] ( 5.1,-0.6) -- ( 5.6,-0.6) -- ( 5.6,0.6) ;
...
```catform
// KV Cache
k_buf: bf16[param.kv_heads, MAX, param.head_dim] = write["g _ e -> g max e", copy](k_r, pos, k_buf)
v_buf: bf16[param.kv_heads, MAX, param.head_dim] = write["g _ e -> g max e", copy](v, pos, v_buf)
```
...
To turn these scores into valid probabilities, we will apply the softmax function as we did in [the Sampler section](01_inference.md#sampler). Prior to doing so, however, we will need to do two things:

1. normalize to maintain unit variance
2. enforce causality

First, the normalization. Because the dot product sums across the contracting dimension, its variance picks up a factor proportional to that dimension's size. To see this, let $\mathtt{x=(x_1,\dots,x_h)}$ and $\mathtt{y=(y_1,\dots,y_h)}$ be vectors where all entries have zero mean and unit variance. The variance of their dot product is then:
