...
In Out[1], indices `0` and `2` both point to position `1`, so under `sum` their values (`10 + 30 = 40`) combine. In Out[2], the `set` reduction leaves position `1` at the target's original `9` (unindexed), and replaces positions `0` and `2` with the source rows.
