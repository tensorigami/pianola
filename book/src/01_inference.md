...
This means we have reduced the problem of defining `infer` to that of defining `generate`. We can depict this relationship in the commutative diagram:
...
class Message:
    ...
    tool_call_id: str = ""
```
We emphasize that all the model did was produce text---it did not actually execute any code to compute the function. Rather, the harness receives the `tool_call` JSON, and treats it as a kind of request that the model made to the harness. It is entirely up to the harness as to what to do with this! A straightforward harness response to a tool call would be to execute the function locally, and then append its result to the context as a `tool` role message, e.g.

```json
{"role": "tool", "tool_call_id": "call_0", "content": "57"}
```

Observe that the `tool_call_id` matches the `id` from the `tool_calls` entry above, telling the model which call this result answers. Upon receiving this result, the model can then resume inference.
...
$$
\mathtt{decode(encode(text)) = normalize(text)}
$$
...
$$
\mathtt{next\_token = sample \circ model}
$$
...
\draw[thick, rounded corners, densely dotted] (-4.8,-1.00) rectangle (4.8,1.00);
\draw[thick, rounded corners, densely dotted] (-3.0,-0.75) rectangle (3.0,0.75);

\draw[thick, rounded corners] (-5.4,-.5) rectangle (-4.2,.5);
\draw[thick, rounded corners] ( 5.4,-.5) rectangle ( 4.2,.5);
...
\draw[thick, rounded corners] (-3.6,-.5) rectangle (-2.4,.5);
\draw[thick, rounded corners] ( 3.6,-.5) rectangle ( 2.4,.5);
...
\draw[thick, rounded corners] (-2.00,-.33) rectangle (-1.2,.33);
\draw[thick, rounded corners] (-0.8,-.33) rectangle (-0.00,.33);
\draw[thick, rounded corners] ( 1.2,-.33) rectangle ( 2.00,.33);

\draw[thick] (-2.4, 0) -- (-2, 0) ;
\draw[thick] ( 2.4, 0) -- ( 2, 0) ;

\draw[thick] (-1.2, .1) -- (-.8, .1) ;
\draw[thick] (-1.2,-.1) -- (-.8,-.1) ;

\draw[thick] (0, .1) -- (.4, .1) ;
\draw[thick] (0,-.1) -- (.4,-.1) ;

\draw[thick] (0.8, .1) -- (1.2, .1) ;
\draw[thick] (0.8,-.1) -- (1.2,-.1) ;
