...
def iota(start: int, stop: int) -> torch.Tensor:
    """Index ramp over the half-open interval [start, stop), as i32."""
    return torch.arange(start, stop, dtype=torch.int32)


# ── BPE pedagogy (chapter 1: tokenization) ───────────────────────────────────
...
def merge_step(tokens: list[str], rule: tuple[str, str]) -> list[str]:
    """Apply one merge rule once across a token sequence."""
    a, b = rule
    out: list[str] = []
    i = 0
    while i < len(tokens):
...
def bpe_encode(text: str, rules: list[tuple[str, str]] | None = None) -> list[str]:
    """Apply all merge rules in order, starting from individual characters."""
    tokens = list(text)
    for rule in rules if rules is not None else merge_rules:
        tokens = merge_step(tokens, rule)
    return tokens
...
_MAP_FN: dict[str, Callable[..., Any]] = {
    ...
    "leq1": lambda *a: (_coerce(a[0]) <= 1.0).float(),
    # Unary
