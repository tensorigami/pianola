...
# ── respond = fetch all the shit ; infer ────────────────────────────────────


def respond(req: Request, B: Backend, state: State) -> tuple[Response, State]:
    """Fetch model + tokenizer + template + sampler from `req`, size the cache
    for this request, run `infer`, return a Response and the updated state.
    ...
    # Pre-size the cache so `infer` runs against an already-sized buffer.
    prompt_tokens = tokenizer.encode(template(context)).ids
    state = model.reserve(state, len(prompt_tokens) + sampler.max_tokens)
    # Snapshot for cached_tokens reporting: LCP of (cached before) vs (cached after).
    before = list(state.cached)
    reply, state = infer(template, tokenizer, sampler, model, context, state)
    cached_tokens = sum(1 for _ in takewhile(lambda ab: ab[0] == ab[1], zip(before, state.cached)))
    return Response(choices=[reply], cached=cached_tokens), state
...
def generate_handler(
    ...
    resp, _ = respond(req, B, catform.zero_state())
    print(prompt + resp.choices[-1].content)
...
def repl_handler(framework: Framework = Framework.torch) -> None:
    ...
    def forward_pass(tokens: list[int]) -> Any:
        logits, _ = catform(tokens, catform.zero_state())
        return logits
...
def _make_app(B: Backend) -> FastAPI:
    ...
    def chat_completions(body: dict[str, Any]) -> dict[str, Any]:
        ...
        if req.model not in states:
            states[req.model] = load(req.model, B)[0].zero_state()
        resp, states[req.model] = respond(req, B, states[req.model])
        return _to_openai(resp, body["model"])
